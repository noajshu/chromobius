...
from clorco.color_code._bell_flagged_planar_color_code_circuits import (
    make_color_code_layout_for_bell_flagged,
    # make_bell_flagged_color_code_circuit,
    # make_bell_flagged_color_code_circuit_round_chunk,
)

# layout = make_color_code_layout_for_bell_flagged(
#   base_data_width=5,
#   single_rgb_layer_instead_of_actual_code=False
# )

# layout.patch.write_svg('./file.svg')


def make_circuit(*, base_data_width: int, basis: str) -> gen.Chunk:
  layout = make_color_code_layout_for_bell_flagged(
    base_data_width=base_data_width,
    single_rgb_layer_instead_of_actual_code=False
  )
  builder = gen.Builder.for_qubits(layout.patch.used_set)
  # Reset the measure qubits
  builder.gate('R', [
    tile.measurement_qubit
    for tile in layout.patch.tiles
    if tile.basis == 'Z'
  ])
  builder.gate('RX', [
    tile.measurement_qubit
  ...
  # First create a bell pair between the ancillas
  builder.tick()
  builder.gate2('CNOT', [
    (tile.measurement_qubit, tile.measurement_qubit + 1)
    for tile in layout.patch.tiles
    if tile.basis == 'X'
  ])

  # Then perform the 2q gates to measure the stabilizer
  if basis == 'X':
    gate = 'CNOT'
  else:
    gate = 'XCZ'
  ...
  builder.gate2(gate, [
    # vertically up
    (tile.measurement_qubit, tile.measurement_qubit - 1j)
    for tile in layout.patch.tiles
    if tile.measurement_qubit - 1j in layout.patch.data_set
  ])
  builder.tick()
  builder.gate2(gate, [
    # vertically down
  ...
  builder.tick()
  builder.gate2(gate, [
    # X goes left and Z goes right
    (tile.measurement_qubit, tile.measurement_qubit + (1 if tile.basis=='Z' else -1))
    for tile in layout.patch.tiles
    if tile.measurement_qubit + (1 if tile.basis=='Z' else -1) in layout.patch.data_set
  ])

  # Unmake the bell pair
  builder.tick()
  builder.gate2('CNOT', [
    (tile.measurement_qubit, tile.measurement_qubit + 1)
    for tile in layout.patch.tiles
  ...
  builder.tick()
  # Measure all the measure qubits
  builder.measure(basis='Z', save_layer='foobar', qubits=[
    tile.measurement_qubit
    for tile in layout.patch.tiles
    if tile.basis == 'Z'
  ])
  builder.measure(basis='X', save_layer='foobar', qubits=[
    tile.measurement_qubit
    for tile in layout.patch.tiles
    if tile.basis == 'X'
  ])
  ...
  for tile in layout.patch.tiles:
    if tile.basis == basis:
      flows.append(
        gen.Flow(
          ...
          measurement_indices=builder.tracker.measurement_indices(
              keys=[gen.AtLayer(tile.measurement_qubit, 'foobar')]
          ),
      ...
      # This is a flow contained entirely within the circuit. I.e., a flag.
      flows.append(
        gen.Flow(
          measurement_indices=builder.tracker.measurement_indices(
              keys=[gen.AtLayer(tile.measurement_qubit, 'foobar')]
  ...
  # Annotate the observable.
  # Note here we are doing x observable
  flows.append(
    gen.Flow(
      start=layout.observables_x[0],
      end=layout.observables_x[0],
      center=0,
    )
  )
  ...
  gen.write_file(
    'file.html',
    gen.stim_circuit_html_viewer(builder.circuit, patch=layout.patch)
  )

  return gen.Chunk(
      circuit=builder.circuit, q2i=builder.q2i, flows=flows,
      discarded_outputs=[
        # Some pauli strings that we do not have here
...
for basis in ['X', 'Z']:
  chunk = make_circuit(base_data_width=5, basis=basis)
  assert chunk.verify() is None

chunkX = make_circuit(base_data_width=5, basis='X')
chunkZ = make_circuit(base_data_width=5, basis='Z')
gen.compile_chunks_into_circuit([chunkX, chunkZ])
...
#     chunk = make_bell_flagged_color_code_circuit_round_chunk(
#         base_data_width=d,
#         initialize=i,
#         basis=b,
#     )
